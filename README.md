...
**Current version 当前版本: 2.1.x**

---

## ✨ The wow: from a messy photo to a finished slide · 从一张潦草的照片到一页成品幻灯片
...
- **2.1** — Document-scan effect (clean white look, dark slides auto-skipped); configurable fonts and summary size. · 扫描件效果（白底质感，暗片自动跳过）；字体与摘要字号可配置。
- **2.0** — Automatic content cropping (frames/spotlights/audience removed), color-agnostic. · 自动裁切（去边框/射灯/观众），颜色无关可泛化。
- **1.0** — Vision reading + consistent-layout PPTX generation. · 视觉读图 + 制式生成 PPT。
