...
Don't want to touch a terminal? Just talk to **WorkBuddy** or **Claude Code** in plain language, and it will clone this repo into the skills directory for you:
...
### Prerequisites (for manual install) · 前置依赖（手动安装时）
...
## ⚙️ Configuration (optional) · 配置项（可选）

All defaults work out of the box — you usually don't need this. The simplest way to change anything is just to **ask the assistant in the conversation** (e.g. "把摘要字号调成 12" / "make the summary font size 12"). Or edit the settings at the top of the generated script directly:

默认值开箱即用，一般用不到这里。最简单的改法是**直接在对话里跟助手说**（比如「把摘要字号调成 12」）。也可以手动改生成脚本顶部的参数：

所有参数都在脚本顶部，改一行即可，下面的渲染代码不用动。
...
Invoke it with the slash command, or just say a trigger phrase · 用斜杠命令调用，或直接说关键词：

```
/conference-buddy
```

Or say any of these in Claude Code / CodeBuddy · 或在 Claude Code / CodeBuddy 里说出任意一句：
